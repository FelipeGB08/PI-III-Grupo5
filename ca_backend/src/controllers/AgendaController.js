...
function normalizarServico(item) {
    ...
    if (nome.length < 3) {
        return { erro: 'Cada servico precisa ter nome com ao menos 3 caracteres.' };
    }
    if (!Number.isFinite(duracao) || duracao < 15 || duracao > 480) {
        return { erro: 'A duracao de cada servico deve ficar entre 15 e 480 minutos.' };
    }
    if (!Number.isFinite(preco) || preco <= 0) {
        return { erro: 'O preco de cada servico deve ser maior que zero.' };
    }
...
function normalizarHorarios(body) {
    const dias = Array.isArray(body.dias_semana) ? body.dias_semana : body.diasSemana;
    const horariosBase = Array.isArray(body.horarios) ? body.horarios : [];
    const resultado = [];

    if (horariosBase.every((item) => typeof item === 'string')) {
        const diasNormalizados = (Array.isArray(dias) && dias.length > 0 ? dias : [1, 2, 3, 4, 5])
            .map(Number)
            .filter((dia) => Number.isInteger(dia) && dia >= 1 && dia <= 7);

        for (const dia of [...new Set(diasNormalizados)]) {
            for (const horario of horariosBase) {
                const horarioNormalizado = AgendaModel.normalizarHorario(horario);
                if (horarioNormalizado) {
                    resultado.push({ dia_semana: dia, horario: horarioNormalizado });
                }
            }
        }
        return resultado;
    ...
    for (const item of horariosBase) {
        ...
        if (Number.isInteger(dia) && dia >= 1 && dia <= 7 && horario) {
            resultado.push({ dia_semana: dia, horario });
        }
    }
...
const AgendaController = {
    buscarPublica: async (req, res) => {
        try {
            ...
            console.error('Erro ao buscar agenda publica:', erro);
            return res.status(500).json({ erro: 'Erro ao carregar agenda do profissional.' });
        }
    },

    buscarMinha: async (req, res) => {
        try {
            const agenda = await AgendaModel.buscarPorProfissional(req.usuarioLogado.id);
            return res.status(200).json(agenda);
        } catch (erro) {
    ...
    salvarMinha: async (req, res) => {
        try {
            const servicosRecebidos = Array.isArray(req.body.servicos) ? req.body.servicos : [];
            if (servicosRecebidos.length === 0 || servicosRecebidos.length > 12) {
                return res.status(400).json({ erro: 'Informe entre 1 e 12 servicos.' });
            }
            ...
            for (const item of servicosRecebidos) {
                ...
                if (normalizado.erro) {
                    return res.status(400).json({ erro: normalizado.erro });
                }
            ...
            if (horarios.length === 0) {
                return res.status(400).json({ erro: 'Informe ao menos um horario valido.' });
            }

            const agenda = await AgendaModel.salvarParaProfissional(req.usuarioLogado.id, {
                servicos,
                horarios,
            });
